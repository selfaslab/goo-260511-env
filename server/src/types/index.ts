...
export interface Finding {
  ...
  /** Optional category tag, e.g. "config" for risky yaml/toml hits. */
...
export interface RepositoryInfo {
  source: ScanSource;
  /** Local absolute path (always present, even for github scans — points to the temp clone for the duration of the scan). */
  projectPath: string;
  repoUrl?: string;
  owner?: string;
  repo?: string;
...
export interface ScanResult {
  ...
  totalFiles: number;
  scannedFiles: number;
  /** Backward-compatible mirror of `score.finalScore` for v1 consumers. */
  severityScore: number;
  riskBreakdown: Record<RiskLevel, number>;
  dependencies: DependencyNode[];
  summary: string;
  scannedAt: string;
  /** Backward-compatible alias of `repository.projectPath`. */
  projectPath: string;
  repository: RepositoryInfo;
  frameworks: FrameworkInfo[];
  score: ScoreBreakdown;
  recommendations: Recommendation[];
  /** Files containing risky configurations (yaml, dockerfile, terraform…). */
